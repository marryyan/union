            ...
            getBasecompanyinfoInfo(){
                ...
                basicFileApis.getBasecompanyinfoInfo(data).then(res => {
                    if(res.status == '200'){
                        ...
                        if (data.unionLetter) {
                            basicFileApis.postSysfileBase({ id: data.unionLetter }).then(response => {
                                if (response.status === 200) {
                                    this.unionLetterSrc = response.result
                                } else {
                ...
                if (response.status === 200) {
                    ...
                    basicFileApis.postbaseBasecompanyinfoUploadunionLetter({
                        ...
                        if (res.status === 200) {
                            ...
                            basicFileApis.postSysfileBase({ id: response.result }).then(wrapResponse => {
                                if (wrapResponse.status === 200) {
                                    this.unionLetterSrc = wrapResponse.result
                                } else {
                                    this.$message.error(wrapResponse.message)
                                }
                            })
                            this.fileList = []
                        } else {
